function matlabbatch = secondlevelanalysisanova_spm8_job(Info, conFilesGrouped, groupNames)
...
%         images with the same size.
    ...
    for i = 1:numel(conFilesGrouped)
        ...
        weight(i) = 1;
        matlabbatch{3}.spm.stats.con.consess{i}.tcon.convec = weight;
    ...
    for i = 1:numel(conFilesGrouped)
        ...
        weight = 1/(numel(conFilesGrouped)-1) * ones(1, numel(conFilesGrouped));
        weight(i) = -1;
        matlabbatch{3}.spm.stats.con.consess{i+numel(conFilesGrouped)}.tcon.convec = weight;
